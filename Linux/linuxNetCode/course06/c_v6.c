...
int main(int argc,char *argv[])
{
	...
	if((he=getaddrinfo()(argv[1]))==NULL)
	{
		exit(1);
	}
	
	//创建一个流式套接口：协议族为ipv4。
	if((connectfd=socket(AF_INET6,SOCK_STREAM,0))==-1)   //函数声明在#include <sys/types.h>      #include <sys/socket.h>中
	{
		printf("%s\n",strerror(errno));
	}
...
/*	
	...
	memset(&server_addr,0,sizeof(server_addr));
*/	
	server_addr.sin6_family=AF_INET6;//协议族为ipv4
	server_addr.sin6_port=htons(8888);//端口号为8888，htons()函数是将主机短字节序列转换为网络短字节序列
	server_addr.sin6_addr=*((struct in6_addr *)he->h_addr);//he->h_addr是char * 类型的，但并非指向一个字符串，指向struct in_addr类型的地址
	...
	char buf[1024]={0};
	int num=0;
	while((num=recv(connectfd,buf,sizeof(buf),0))!=0)//循环接受消息，保证消息完全接受，因为TCP是无边界的
	{
		buf[num]=0;
		puts(buf);
	}
